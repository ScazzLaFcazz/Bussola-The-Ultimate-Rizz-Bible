...
export function draftSystem(style, opener = false) {
...
/**
 * Prompt material lives in docs/*.md so it stays auditable and editable without a
 * code change. Returns '' on failure; callers refuse rather than running a prompt
 * with half its instructions missing.
 *
 * HOUSE_RULES deliberately does NOT live in a file. A fetch that fails, or a file
 * someone edits, must not be able to weaken the limits.
 */
async function loadDoc(path) {
  ...
  try {
    ...
    /* offline or blocked: caller decides what to do with '' */
  }
  docCache.set(path, text);
  return text;
}

export const loadRubric = () => loadDoc('docs/signals.md');
export const loadStyle = () => loadDoc('docs/style.md');
